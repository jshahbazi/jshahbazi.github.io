...
for required in ("index.html", "about/index.html", "Hello-World.html", "404.html", "style.css", "feed.xml", "sitemap.xml"):
    assert (ROOT / required).is_file(), f"Missing output: {required}"

for excluded in ("_drafts", "post-template.html", "README.md", "Gemfile", "Gemfile.lock", "scripts", "vendor", ".github", "AGENTS.md"):
...
print(f"Checked {len(pages)} pages, internal links, {len(items)} RSS items, sitemap, and production exclusions.")
